...
<div class="bg-white rounded-lg shadow-inner p-4 text-center border-red-200 border">
  ...
  <div class="flex justify-center gap-4">
    <?php if ($latestOM): ?>
      <a href="monitoring_detail.php?npk=<?= urlencode($npk) ?>&machine=<?= $latestOM['sub_id'] ?>&process_id=<?= $latestOM['process_id'] ?>"
         class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded shadow">
        Latest OM
      </a>
    <?php endif; ?>
    <?php if ($latestIM): ?>
      <a href="monitoring_detail.php?npk=<?= urlencode($npk) ?>&machine=<?= $latestIM['sub_id'] ?>&process_id=<?= $latestIM['process_id'] ?>"
