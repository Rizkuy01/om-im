...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sub_workstation_id > 0 && $part_number !== '' && $uploaded) {
        ...
        $stmt = $connIMOM->prepare("SELECT id, path_name FROM {$tableName} WHERE sub_workstation_id = ? AND part_number = ?");
        ...
        if (isset($_POST['replace']) && $existing) {
            if (move_uploaded_file($uploaded['tmp_name'], $targetFile)) {
                $stmt = $connIMOM->prepare("UPDATE {$tableName} SET file_name=?, path_name=? WHERE sub_workstation_id=? AND part_number=?");
                $stmt->bind_param("ssis", $newFileName, $basePath, $sub_workstation_id, $part_number);
                $stmt->execute();
                $stmt->close();
            }
        } 
